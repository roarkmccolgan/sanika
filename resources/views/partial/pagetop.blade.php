<div class="bg-black py-2 text-white text-sm font-bold relative z-50 sm:h-10 sm:overflow-visible">
		
	<div class="container mx-auto flex flex-wrap items-start justify-between px-2 sm:px-0">
		...
		<transition name="slide">
			<a href="/news/{{ $news->first()->category->alias }}/{{ $news->first()->alias }}" ref="newsItem" class="newsItem sm:order-none w-full sm:flex-1 hidden no-underline mt-2 mr-0 sm:mr-8 sm:mt-0 p-2 w-full sm:w-2/5 text-white bg-white rounded-b border-b border-grey shadow pointer-events-auto" style="background: rgb(201,0,0);background: linear-gradient(0deg, rgba(201,0,0,1) 40%, rgba(136,45,45,1) 90%, rgba(89,0,0,1) 100%);" v-show="showNews" v-cloak>
